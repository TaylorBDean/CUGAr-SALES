...
class MCPRunner:
    ...
    def __init__(
        self, *, timeout: float = 15.0, retries: int = 1, client: Optional[httpx.Client] = None
    ) -> None:
    ...
    def invoke(
        self, tool: MCPTool, payload: Dict[str, Any], *, headers: Optional[Dict[str, str]] = None
    ) -> InvocationResult:
            ...
            try:
                ...
                if response.is_success:
                    ...
                    return InvocationResult(
                        ok=True, status_code=response.status_code, output=body, raw=response
                    )
                return InvocationResult(
                    ok=False, status_code=response.status_code, output=response.text, raw=response
                )
            except httpx.HTTPError as exc:
                logger.warning(
                    "MCP invocation error for %s (attempt %s/%s): %s",
                    tool.display_name,
                    attempt + 1,
                    self.retries + 1,
                    exc,
